...
function processDuplicateFree(list, cb) {
    let newList = list.filter(function(el, i, arr) {
...
function getFullNames(runners) {
    const names = runners.map(element => {
...
function firstNamesAllCaps(runners) {
    const caps = runners.map(element => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
    const tShirts = runners.filter(function(item) {
...
function tallyUpDonations(runners) {
    const donationSum = runners.reduce(function(accumulator, currentValue) {
 ...
 * etc
 */
function counterMakerWithLimit(x) {
    let count = 0;
    return function() {
        if (x === count) {
            count = 0;
